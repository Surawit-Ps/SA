...
// GetWagesByWorkID handles fetching Wages details by workID
func GetWagesByWorkID(c *gin.Context) {
    workID := c.Param("workID") // รับค่า workID จาก path parameters
    log.Println("Received workID: ", workID) // Log เพื่อตรวจสอบค่า workID

    // ตรวจสอบว่าค่าที่ได้รับมีรูปแบบที่ถูกต้อง
    if workID == "" {
        c.JSON(http.StatusBadRequest, gin.H{"error": "WorkID cannot be empty"})
        return
    }
    ...
    // ค้นหาข้อมูล work ตาม workID
    result := db.Where("work_id = ?", workID).First(&work)

    if result.Error != nil {
        log.Println("Error: ", result.Error) // Log ข้อผิดพลาดเพื่อตรวจสอบ
        if result.Error == gorm.ErrRecordNotFound {
            c.JSON(http.StatusNotFound, gin.H{"error": "Work not found for the given WorkID"})
        } else {
            c.JSON(http.StatusInternalServerError, gin.H{"error": "An error occurred while retrieving the work"})
        }
        return
    }

    log.Println("Found work: ", work) // Log ข้อมูล work เพื่อตรวจสอบ
    c.JSON(http.StatusOK, gin.H{"wages": work.Wages}) // ส่งค่า wages กลับในรูปแบบ JSON
}
